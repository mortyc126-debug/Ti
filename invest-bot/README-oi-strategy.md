...
invest-bot/
  ...
  data/archive.json            ← состояние risk.py / mega_alerts.py / archive.py,
                                  переживает рестарт
```
...
**Известное ограничение**: `risk.py` использует `point_value=1.0` во всех
расчётах риска — это верно для акций (1 пункт цены = 1 рубль), но для
фьючерса фактическая стоимость пункта цены другая (зависит от
`min_price_increment_amount` контракта). Сейчас это не учитывается —
риск-метрики (R, stop distance) для фьючерсов будут не совсем точны,
хотя сам вход/выход по сигналу и размер позиции по ГО считаются верно.
