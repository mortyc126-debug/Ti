...
## Гейт по исторической точности сигнала (`signal_gate.py`)

Ещё один, отдельный от composite-скора фильтр — порт `labSignalScreener` /
`computeIndicator` из `oi_lab.html` (не путать с oi-signal-v10.html, откуда
уже портированы `oi_layers.py`/`tradestats.py` — это другой, более новый
инструмент). В отличие от 29 методов composite, здесь не скор, а жёсткий
гейт "не входить": блокирует вход, только если сигнал, который прямо сейчас
активен на тикере, на этом же самом тикере эмпирически исторически не
работал.

Три подтверждённых на чистых данных триггера (роллы фьючерса исключены из
статистики) — по формулировке автора все три шортовые:
- **trendDown** — тренд-истощение: цена росла N баров подряд, лонг физлиц
  (физ+юр суммарно) у исторического экстремума → ждём коррекцию, шорт.
- **exh** — истощение после выброса вверх: пробой диапазона накопления ПО
  позиции физлиц (подтверждение, не сквиз против неё) → шорт.
- **longAggr** — лонг физлиц набирался агрессивно (нижний край
  перцентильного ранга давления потока позиций) → шорт.

Четвёртый триггер из oi_lab.html ("сетап+истощение") помечен там же не
подтверждённым после чистки данных (EV −3.3, только для наблюдения) —
в гейт сознательно не включён, никогда не блокирует.

Для каждого активного сейчас триггера гейт смотрит на **свою историю именно
этого тикера**: сколько раз триггер срабатывал раньше и как часто цена
после этого действительно шла в его сторону за 3 дня. Три исхода:
- **N≥8 и точность ≥60%** — торгуемое, гейт молчит;
- **N≥8 и точность <60%** — "против статистики", гейт **блокирует** вход
  в эту сторону, даже если composite-сигнал прошёл порог;
- **N<8** — своей истории мало, доверять нельзя → гейт тоже молчит (не
  блокирует "на всякий случай" при отсутствии доказательств — то же
  правило, что у остальных фильтров качества выше).

Причина блокировки логируется вместе с асимметрией исход/риск: медианный
ход, когда сигнал прав, отдельно от медианного хода, когда неправ — не
"средняя доходность", а явное разделение вознаграждения и риска (как в
`labSignalScreener`).

**Данные и расчёт**: гейт читает `data/oi_daily.json` — тот же файл, что
уже пишет `oi_layers.py` (физ/юр контракты по FutOI) — новых сетевых
запросов не требует. Пересчитывается (`recalibrate`) раз в торговый день
в начале `Trader.__setup_trade_day` (по ВСЕМ тикерам с данными в файле,
не только сегодняшним `settings.ini` — так пуловая статистика честнее).
Результат калибровки переживает рестарт бота (`data/signal_gate.json`).

Урезание порта: `computeIndicator` в oi_lab.html помимо полей, которые
реально используют три триггера выше, считает ещё force/conviction/
velocity_decel/liquidity_conf/weighted_imbalance — они не влияют ни на
один из трёх триггеров (проверено по исходнику), поэтому в `signal_gate.py`
не портированы — не приближение формул, а честный пропуск мёртвого для
гейта кода. Полный пайплайн (rolling percentile rank, ATR-адаптивное
подтверждение фазы накопления через гистерезис) портирован 1:1.
...
invest-bot/
  ...
  signal_gate.py                 ← гейт по исторической точности сигнала на
                                  тикере (порт labSignalScreener из oi_lab.html),
                                  data/signal_gate.json — калибровка раз в день
  ...
  data/mega_alerts.json,
  data/archive.json,
  data/signal_gate.json        ← состояние risk.py / mega_alerts.py / archive.py /
                                  signal_gate.py, переживает рестарт
```
...
### Автоторговля тикерами вне settings.ini (`[MEGA_ALERTS] AUTO_TRADE=1`)

Если включено — для каждого тикера из `extra_tickers` (только рынок акций
`eq`, futures-алёрты `fo` идут только в уведомление) бот:
1. резолвит figi через `InstrumentService.share_by_ticker` (TQBR);
2. создаёт `OICompositeStrategy` с дефолтными параметрами из `[MEGA_ALERTS]`
   (`SIGNAL_THRESHOLD/LONG_TAKE/.../SIGNAL_ONLY`);
3. **запрашивает историю свечей** тикера за `HISTORY_DAYS` дней
   (`MarketDataService.get_candles_history`, 5-минутные бары, чанками по
   дню — ограничение Tinkoff API) и прогоняет её через стратегию
   (`OICompositeStrategy.warmup`), чтобы досчитать всё недостающее
   (скользящие окна индикаторов, режим рынка, EWA-веса методов) —
   тикер стартует не с пустого состояния, а с уже «прогретой» историей;
4. **бэктестит качество** на этой же истории —
   `OICompositeStrategy.backtest_quality`: скользящим окном эмулирует
   вход по каждому пересечению порога композита и считает виртуальный
   MFE/MAE (Maximum Favorable/Adverse Excursion) на `MFE_MAE_BARS` баров
   вперёд, возвращает средний `quality = MFE/(MFE+MAE)` по всем виртуальным
   сделкам и их количество;
5. **разрешает реальные ордера только если бэктест хороший**: если
   виртуальных сделок ≥ `BACKTEST_MIN_TRADES` и `quality` ≥
   `BACKTEST_QUALITY_MIN`, стратегия переключается в `SIGNAL_ONLY=0`
   (`strategy.set_signal_only(False)`) и торгует по-настоящему; иначе
   остаётся в режиме «только сигналы» в Telegram, как и раньше;
6. прогоняет через ту же проверку торговой готовности (`otc_flag`,
   `buy/sell/api_trade_available_flag`), что и обычные тикеры;
7. подключает к ОИ-поллеру (`oi_layers.py`) и микроструктуре (`tradestats.py`)
   так же, как сконфигурированные тикеры — полный 29-методный композит.

`MAX_TICKERS` ограничивает, сколько новых тикеров можно добавить за день
(по умолчанию 5). Список не сохраняется на диск — пересобирается с нуля
каждый торговый день из текущего срез аномалий, веса EWA для новых тикеров
стартуют с дефолтных и довоспитываются на истории через `warmup` (история
весов копится в `oi_weights.json` по figi, как у любого другого тикера).
По умолчанию `SIGNAL_ONLY=1` в `[MEGA_ALERTS]` — это потолок: даже если
бэктест хороший, тикер не превысит этот режим; чтобы вообще разрешить
переключение в реальную торговлю по бэктесту, нужно `SIGNAL_ONLY=0`.

### Архив расчётов (`archive.py`, `data/archive.json`)

После каждого торгового дня (`Trader.__archive_today`) для каждой
активной стратегии (и сконфигурированных в `settings.ini`, и
авто-найденных по MEGA-ALERTS) бот забирает её последний снэпшот
(`OICompositeStrategy.last_snapshot`) — composite-сигнал, разбор по всем
29 методам, текущий режим рынка, rolling-quality EWA — и пишет в
персистентный архив `ArchiveStore` (`data/archive.json`), вместе с тем,
торговался ли тикер реально (`live`) и, для авто-найденных — качеством и
числом сделок бэктеста. Это и есть «рабочий, который считает всё для
архива»: каждый день добавляется снэпшот по каждому активному тикеру,
...
Идея: на акциях движения маленькие, а комиссии большие; фьючерс на тот же
базовый актив даёт плечо и обычно дешевле по комиссии. Бот может торговать
фьючерсом ВМЕСТО акции на тех же тикерах, что уже настроены в `STRATEGY_*`.

Включается секцией `[FUTURES_TRADING]` в `settings.ini`:
- `ENABLED=1` — включить (по умолчанию `0`, акции торгуются как раньше);
- `BASE_TICKERS=SBER,GAZP,LKOH` — список базовых активов, для которых
  искать фьючерс.

**`InstrumentService.future_by_base_ticker(base_ticker)`** — находит среди
всех фьючерсов на бирже (`client.instruments.futures`) тот, у которого
`basic_asset == base_ticker`, `api_trade_available_flag` и ближайшая
непросроченная `expiration_date`. Контракт переизбирается каждый торговый
день — если прежний истёк, бот сам найдёт следующий.

**ГО (гарантийное обеспечение) — из API, не из settings.ini.** ГО меняется
день в день, поэтому `margin_per_lot` берётся на каждый вызов как
`max(initial_margin_on_buy, initial_margin_on_sell)` из ответа API. Это
кладётся в `StrategySettings.margin_per_lot` + `is_future=True`.

**Сигнальные настройки переиспользуются**: если для базового тикера уже
есть `STRATEGY_<TICKER>_SETTINGS` (порог сигнала, take/stop) — фьючерс
торгуется с теми же значениями; если тикера нет среди `STRATEGY_*` —
берутся дефолты из `[MEGA_ALERTS]`.

**Размер позиции считается по ГО, а не по цене×лот**: для фьючерса
`__open_position_lots_count` делит свободный кэш на `margin_per_lot`
вместо `price * lot_size` — иначе для акции плечо игнорировалось бы и
позиция была бы в разы меньше, чем позволяет реальное ГО.

**Закрытие позиций**: Tinkoff API возвращает фьючерсные позиции в отдельном
поле `PositionsResponse.futures`, НЕ в `.securities` (даже если позиция
открыта тем же `post_market_order`). `OperationService.positions_futures`
и объединение со `positions_securities` в `__close_position_by_figi`
добавлены отдельно — без этого открытые фьючерсы никогда не закрылись бы
автоматически в конце дня.

**Стоимость пункта цены (`point_value`)**: для акции 1 пункт цены = 1 рубль,
для фьючерса — иначе (`min_price_increment_amount / min_price_increment`
контракта). `future_by_base_ticker` считает это значение из API и кладёт в
`StrategySettings.point_value`, а `risk.py` использует его во всех расчётах
риска (`open_position`/`update_stop`/`check_exit` — везде через
`strategy.settings.point_value`), так что R, stop distance и risk_rub для
фьючерса считаются в реальных рублях, а не как для акции.

## Адаптивный выход (ADAPTIVE_EXIT=1)

По умолчанию выход — фиксированный stop/take уровень сигнала стратегии.
При `ADAPTIVE_EXIT=1` (env) фиксированные уровни игнорируются для **всех**
открытых позиций (не только тикеров с известным squeeze) — вместо них
работает `risk.check_exit`: трейлинг-стоп (Chandelier), переставляемый
безубыток после 1R прибыли, giveback-защита пика, плюс для шортов —
доп. ранний выход при squeeze-риске (реальный squeeze_score из
`oi_layers.py`, не статичный порог). Стоп двигается на каждом баре —
позиция не закрывается по первому касанию исходного take_profit, может
забрать большее движение, если тренд продолжается.
