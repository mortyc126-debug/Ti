...
3. прогоняет через ту же проверку торговой готовности (`otc_flag`,
   `buy/sell/api_trade_available_flag`), что и обычные тикеры;
4. подключает к ОИ-поллеру (`oi_layers.py`) и микроструктуре (`tradestats.py`)
...
стартуют с дефолтных (история весов копится в `oi_weights.json` по figi,
как у любого другого тикера). По умолчанию `SIGNAL_ONLY=1` — авто-найденные
тикеры сначала только шлют сигналы в Telegram, без реальных ордеров;
выставить `SIGNAL_ONLY=0` в `[MEGA_ALERTS]`, когда готовы давать им
торговать по-настоящему.
