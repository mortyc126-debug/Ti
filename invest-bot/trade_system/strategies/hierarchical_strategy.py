...
СТАТУС: используется только для бэктеста/сравнения через strategy_factory.py.
В live-торговле активна OICompositeStrategy (oi_composite_strategy.py).
Не удалять: нужна для A/B-бэктестов и как референс структурной логики
(D1→H1→M5), частично перенесённой в MKT_STRUCTURE/SPRING.
