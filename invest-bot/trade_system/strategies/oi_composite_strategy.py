...
def score_elliott_wave(candles: list[HistoricCandle]) -> float:
    ...
    v2 после реального бэктеста (score_methods.py): v1 давала 95866
    срабатываний на 377 тикеров за 180 дней (role=noise, win 46%) — по сути
    ловила любой мало-мальски волнистый зигзаг и стреляла с первого же тика
    после волны 4, три мягких правила из пяти были не отказом, а лёгким
    штрафом к и без того невысокому порогу. Ужесточено до почти всех
    жёстких правил + задержка первого срабатывания, чтобы ловить именно
    РЕДКИЙ, канонiчный паттерн, а не общий трендовый шум:
      - порог зигзага 1.2×ATR → 1.8×ATR (меньше пивотов от шума)
      - нужен ПОЛНЫЙ P0 (5-й пивот назад), не суррогат от края окна
      - волна 2 не пробивает старт волны 1 — ЖЁСТКО (как и было)
      - волна 3 короче волны 1 — ЖЁСТКО отказ (было: −0.15 к уверенности)
      - волна 4 заходит на территорию волны 1 — ЖЁСТКО отказ (было: −0.15)
      - волна 2 вне зоны 23.6-88.6% волны 1 — ЖЁСТКО отказ (было: ±0.10)
      - волна 5 должна набрать ≥25% волны 1, прежде чем сигнал вообще
        появится (было: сигнал уже на первом баре волны 5)

    Сигнал (только когда все жёсткие правила выполнены):
      Волна 5 в диапазоне 0.25..1.0×волна1 → продолжение по тренду,
      сила растёт с прогрессом.
      1.0..1.68×волна1 → сигнал угасает к нулю (обычная зона завершения).
      > 1.68×волна1 → контр-сигнал против импульса (псевдо-растяжение,
      ждём ABC-коррекцию).
    """
    _PIVOT_MULT = 1.8
    ...
    try:
        ...
        piv = _zigzag_pivots(win, atr_abs, _PIVOT_MULT)
        # нужен ПОЛНЫЙ P0 — суррогат от края окна (win[0]) давал искажённую
        # длину волны 1 и ложные срабатывания у начала анализируемого окна
        if len(piv) < 5:
            return 0.0
        p0_i, p0_p, k0 = piv[-5]
        p1_i, p1_p, k1 = piv[-4]
        ...
        p3_i, p3_p, k3 = piv[-2]
        p4_i, p4_p, k4 = piv[-1]
        # должны чередоваться: лоу,хай,лоу,хай,лоу (или наоборот)
        if not (k0 == -k1 == k2 == -k3 == k4):
            return 0.0
        ...
        # ── Жёсткие правила Эллиотта — любое нарушение отклоняет счёт целиком ──
        if direction > 0 and p2_p <= p0_p:            # волна 2 пробила старт волны 1
            return 0.0
        if direction < 0 and p2_p >= p0_p:
            return 0.0
        if wave3 < wave1:                               # волна 3 короче волны 1
            return 0.0
        if direction > 0 and p4_p < p1_p:               # волна 4 зашла на территорию волны 1
            return 0.0
        if direction < 0 and p4_p > p1_p:
            return 0.0
        ratio2 = wave2 / wave1
        if not (0.236 <= ratio2 <= 0.886):               # волна 2 вне разумной Фибо-зоны
            return 0.0
        ...
        if wave5 <= 0 and abs(last_close - p4_p) >= atr_abs * _PIVOT_MULT:
            return 0.0

        # Все жёсткие правила прошли — паттерн валиден, уверенность только
        # от "необязательных" характерных черт (растяжение волны 3)
        confidence = 0.55
        if wave3 >= wave1 * 1.5:
            confidence += 0.15
        ...
        # рано звать сигнал — волна 5 едва тронулась с места
        if w5_ratio < 0.25:
            return 0.0
        if w5_ratio < 1.0:
            # волна 5 в процессе, ещё не набрала типичный размер — едем по
            # тренду, сила растёт по мере приближения к волне1
            progress = (w5_ratio - 0.25) / 0.75
            return round(direction * confidence * (0.35 + 0.65 * progress), 4)
        if w5_ratio <= 1.68:
            # обычная зона завершения волны 5 — сигнал по тренду угасает к нулю
            fade = 1.0 - (w5_ratio - 1.0) / 0.68  # 1.0 → 0.0
            return round(direction * confidence * fade * 0.5, 4)
