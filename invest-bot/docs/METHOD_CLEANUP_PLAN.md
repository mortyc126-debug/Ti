...
> - ✅ `score_methods.py` теперь пишет по тикеру `liq_mln`/`vol_pct` в
>   per-ticker CSV (`--out`) и в конце — сводку «зависимость edge (d) метода
>   от ликвидности/волатильности»: Spearman(d, ликв/волат) + медиана d по
>   третям ликвидности (ловит знак-флип, SBER-класс). То есть у каждого
>   метода видно не только режим, но и на каких тикерах он работает.
...
> - ✅ Все ТРИ скрипта Шага 1 в одной логике по ликвидности/волатильности:
>   `lag_analysis` (агрегат `--all`) в конце печатает «зависимость ЛАГА
>   метода от ликвидности» (Spearman + лаг по третям); `redundancy_analysis`
>   (`--all`) — «зависимость quality метода от ликвидности» (Spearman +
>   quality по третям). Плюс `score_methods` — «зависимость edge (d)».
>   Теперь по каждому методу видно режим + знак + на каких тикерах работает.
