...
Целевая переменная: fwd_ret_3 / fwd_ret_6 / fwd_ret_12 (выбор через --target).
...
Использование:
    ...
    python bar_rule_miner.py AFKS AFLT SBER --days 365 --target fwd_ret_6
...
def _build_bar_features(rows: list[dict]) -> tuple[list[str], list[list[float]], list[float], list[float], list[str]]:
    """
    Возвращает (feature_names, X, fwd3, fwd6, regimes).
    ...
    for r in rows:
        try:
            f3 = float(r["fwd_ret_3"])
            f6 = float(r.get("fwd_ret_6") or 0)
            vol = float(r["volume"])
            o   = float(r["open"])
            h   = float(r["high"])
            lo  = float(r["low"])
    ...
    return feature_names, X, fwd3, fwd6, regimes
...
def mine_ticker(ticker: str, days: int | None, max_depth: int, target: str) -> dict | None:
    ...
    feature_names, X, fwd3, fwd6, regimes = _build_bar_features(rows)
    y_all = fwd3 if target == "fwd_ret_3" else fwd6

    n_bars = len(X)
    print(f"{ticker}: {n_bars} баров, {len(feature_names)} фичей, цель={target}")
...
def main():
    ...
    parser.add_argument("--target",    default="fwd_ret_3",
                        choices=["fwd_ret_3", "fwd_ret_6"], help="целевая переменная")
