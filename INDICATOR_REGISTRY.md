...
| `wr` | — | ✅ `_ilAltWR` (гейт диапазона + устойчивый экстремум=продолжение) | новое | ✅ 3.1 |
...
| `cmo` | — | ✅ `_ilAltCMO` (обрыв окна, CMO≡2·RSI−100) | новое | ✅ 3.2 |
| `tsi` | — | ✅ smooth=f(len)≈len/2 | — | ✅ 2.11 |
| `ult_osc` | — | ✅ `_ilAltUO` (рассогласование raw7/14/28) | новое | ✅ 3.3 |
...
| `donchian` | DONCHIAN | ✅ `_ilAltDonchian` (свежесть пробоя + ширина канала с лагом) | новое | ✅ 3.7 |
...
| `elder_ray` | — | ✅ `_ilAltElderRay` (обусловленное чтение bear/bull + ATR норм.) + фикс мёртвой -1 | новое | ✅ 3.4 |
| `elder_impulse` | — | ✅ `_ilAltElderImpulse` (LAG=3, HYST=2, amp модулятор) + фикс мёртвых ±1 | новое | ✅ 3.5 |
...
| indlab id | стратегия | альт сейчас | лучше где | статус |
|---|---|---|---|---|
| `nw` | NADARAYA_WATSON | dsp flip-fade (общий — обоснованно, нет K/variance аналога) | — | ✅ 2.12 (оставлен как есть по ТЗ) |
| `kalman` | — | ✅ `_ilAltKalmanFlip` (K как сила флипа) + фикс Б.6 (rolling R/Q) | новое | ✅ 2.12 |
| `gpr` | — | ✅ `_ilAltGprFlip` (ширина интервала) + variance reuse из карточки (Б.7) | новое | ✅ 2.12 |
| `cusum` | CHANGE_POINT (близко) | ✅ градация по свежести i-lastCp → шкала ±2/±1/0 | — | ✅ 2.13 |
