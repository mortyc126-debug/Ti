...
async function _repD1Load(){
  ...
  try{
    ...
    for(var ci = 0; ci < issuers.length; ci++){
      ...
      if(!ci2.inn) continue;
      var existId = null;
      var rdb = window.reportsDB || (window.reportsDB = {});
      for(var eid in rdb){
      ...
      if(!existId){
        var nid = 'i_' + ci2.inn;
        if(!rdb[nid]){
          rdb[nid] = { name: ci2.name || ci2.short_name || ci2.inn, inn: ci2.inn, ind: _d1SectorToInd(ci2.sector), periods: {} };
          added++;
        }
      } else {
        var ex = rdb[existId];
        if(!ex.ind || ex.ind === 'other') ex.ind = _d1SectorToInd(ci2.sector);
        if(!ex.name && (ci2.name || ci2.short_name)) ex.name = ci2.name || ci2.short_name;
        updated++;
      }
    }
    ...
    // 3. Подтянуть отчёты для эмитентов без периодов И ещё не проверенных
    var toFetch = [];
    var rdb2 = window.reportsDB || {};
    for(var eid2 in rdb2){
      var e2 = rdb2[eid2];
      // Пропускаем: нет ИНН, уже есть периоды, уже проверялись в D1 и вернули пусто
    ...
    try{ saveState(); }catch(e){ console.warn('saveState:', e); }
    repRenderIssuerList();
    // Итоговый подсчёт базы
    ...
    var summary = added > 0 ? '+' + added + ' эм.' : '';
    var loadedStr = loaded > 0 ? (summary ? ', ' : '') + '+' + loaded + ' отч.' : '';
    var statusStr = '✓ ' + (summary + loadedStr || 'без изменений') + ' | база: ' + withPeriods + '/' + totalIss;
    setBtn('✓ ' + withPeriods + '/' + totalIss + ' с отчётами');
    setLog(statusStr + (errors ? ' (ошибок: ' + errors + ')' : ''));
