...
async function _repD1Load(){
  ...
  function setBtn(t, ok){ if(btn){ btn.textContent = t; btn.disabled = !ok; } }
  setBtn('↻ каталог…', false);
  try{
    var base = _getD1Base();
    // 1. Каталог — список эмитентов и облигаций
    var r = await fetch(base + '/catalog');
    if(!r.ok) throw new Error('catalog ' + r.status);
    var cat = await r.json();
    _d1Catalog = cat;
    var issuers = cat.issuers || [];
    ...
    for(var ci = 0; ci < issuers.length; ci++){
      var ci2 = issuers[ci];
      if(!ci2.inn) continue;
      // Найти существующую запись по ИНН
      var existId = null;
      ...
      if(!existId){
        ...
        if(!rdb[nid]){
          rdb[nid] = {
            name: ci2.name || ci2.short_name || ci2.inn,
            inn: ci2.inn,
            ind: _d1SectorToInd(ci2.sector),
            periods: {}
          };
          window.reportsDB = rdb;
          added++;
        }
      } else {
        // Обновить имя и отрасль если есть новые данные
        var ex = rdb[existId];
        if(!ex.ind || ex.ind === 'other') ex.ind = _d1SectorToInd(ci2.sector);
      }
    }

    // 3. Подтянуть отчёты для эмитентов без периодов
    var toFetch = [];
    var rdb2 = window.reportsDB || {};
    for(var eid2 in rdb2){
      var e2 = rdb2[eid2];
      if(e2 && e2.inn && Object.keys(e2.periods || {}).length === 0){
        toFetch.push({ id: eid2, inn: e2.inn });
      }
    }
    var loaded = 0;
    var total = toFetch.length;
    for(var ti = 0; ti < total; ti += 5){
      var batch = toFetch.slice(ti, ti + 5);
      setBtn('↻ ' + ti + '/' + total, false);
    ...
    repRenderIssuerList();
    setBtn('✓ +' + added + ' эм, ' + loaded + ' отч.', true);
    setTimeout(function(){ setBtn('↓ D1', true); }, 5000);

  }catch(e){
    console.error('D1 load:', e);
    setBtn('✗ ' + (e.message||'ошибка'), true);
    setTimeout(function(){ setBtn('↓ D1', true); }, 4000);
  }
}
