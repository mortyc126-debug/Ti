...
// Инжектируем CSS — только улучшения поверх существующего дизайна
(function(){
  var st = document.createElement('style');
  st.textContent = [
    // Карточки: чёткое разделение, левая цветная полоска
    '#rep-sidebar-list{display:flex;flex-direction:column;gap:4px;padding:6px 8px;}',
    '.rep-issuer-item{',
    ...
    '}',
    '.rep-issuer-item:hover{background:var(--s2) !important;border-color:var(--border2) !important;}',
    '.rep-issuer-item[data-active="1"]{',
    '  background:var(--acc-dim) !important;',
    '  border-color:var(--acc) !important;',
    '  border-left-color:var(--acc) !important;',
    '}',
    '.rep-iss-name{font-size:.68rem !important;font-weight:600 !important;line-height:1.3 !important}',
    '.rep-iss-mults{display:flex;flex-wrap:wrap;gap:3px;margin-top:4px}',
    '.rep-iss-pill{font-size:.55rem;font-family:var(--mono);padding:1px 6px;',
    ...
    // Плитки Запас прочности / Качество баланса — светлее для визуального отделения
    '#rep-issuer-header [title*="Запас прочности"]:not(label),',
    '#rep-issuer-header [title*="Altman"]{',
    '  background:var(--s2) !important;border-color:var(--border2) !important;',
    '  border-radius:8px !important;margin-top:6px !important;}',
    // Кнопки — фиолетовый акцент
    '.btn-p,.btn-primary{border-color:var(--acc);color:var(--acc);background:var(--acc-dim)}',
    '.btn-p:hover,.btn-primary:hover{background:var(--acc);color:var(--bg)}',
    '.btn.active,.btn-sm.active{border-color:var(--acc);color:var(--acc);background:var(--acc-dim)}',
    '.rep-tf-btn.active{border-color:var(--acc);color:var(--acc);background:var(--acc-dim)}',
    // Динамика: бары → фиолетовый
    '.dyn-bar-pos{background:var(--acc) !important}',
    '.dyn-bar-fill{background:var(--acc) !important}',
  ].join('');
