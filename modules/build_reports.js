...
const newSidebar = `<div id="rep-sidebar" style="width:400px;flex-shrink:0;position:sticky;top:8px;max-height:calc(100vh - 80px);display:flex;flex-direction:column;background:var(--s1);border:1px solid var(--brdm);overflow:hidden">
...
.iss-name{font-size:13px;font-weight:600;color:var(--t);line-height:1.3;margin-bottom:2px;
...
.d-bar{height:4px;background:rgba(255,255,255,0.05);position:relative;overflow:hidden;max-width:200px}
...
.r-track{height:5px;background:rgba(255,255,255,0.05);position:relative;overflow:hidden;max-width:240px}
...
/* ── Форма периода: поля и модалки ───────── */
.field label{display:block;font-size:11px;letter-spacing:.06em;text-transform:uppercase;color:var(--t3);margin-bottom:5px;font-family:var(--sans);font-weight:600}
.field input,.field select,.field textarea{width:100%;background:var(--bg);border:1px solid var(--brdp);color:var(--t);font-family:var(--mono);font-size:13px;padding:8px 11px;outline:none;transition:border-color .2s;resize:vertical}
...
.gap-row{display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap}
.g2{display:grid;grid-template-columns:1fr 1fr;gap:12px}
.g3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px}
.modal{background:var(--s1);border:1px solid var(--brdm);width:560px;max-width:96vw;max-height:92vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.85)}
.modal-hdr{padding:16px 22px;border-bottom:1px solid rgba(255,0,128,0.2);display:flex;align-items:center;justify-content:space-between;font-size:14px;font-weight:700;color:var(--t);letter-spacing:0}
.modal-body{padding:20px 22px;display:flex;flex-direction:column;gap:14px}
.modal-ftr{padding:14px 22px;border-top:1px solid var(--brd);display:flex;gap:8px;justify-content:flex-end;background:rgba(255,255,255,0.012)}
.modal-x{background:none;border:none;color:var(--t3);cursor:pointer;font-size:18px;padding:0 2px;line-height:1;opacity:.6;transition:opacity .12s}
.modal-x:hover{opacity:1;color:var(--t)}
/* подавить крошечные inline font-size в теле модала */
#modal-rep-period .modal-body [style*="font-size:.5"],
#modal-rep-period .modal-body [style*="font-size:.6"]{font-size:11px!important;line-height:1.55!important}
#modal-rep-period .modal-body strong{font-size:12px}

/* ── Зоны динамики: читаемая типографика ─ */
.zone-name{font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--t4)}
.zone-si{display:flex;align-items:center;gap:3px;font-size:11px;font-family:var(--mono)}
.zone-sl{color:var(--t3);font-size:10px;margin-right:2px}
.d-lbl{font-size:13px;color:var(--t2);display:flex;align-items:center;gap:5px;flex:1}
.d-lbl.bold{color:var(--t);font-weight:600}
.d-vals{display:flex;align-items:baseline;gap:4px;flex-shrink:0;font-family:var(--mono);font-size:12px;margin-left:auto}
.r-norm{margin-left:auto;font-size:10px;color:var(--t3);font-family:var(--mono)}
.score-lbl{font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--t3);margin-bottom:5px}
.score-val{font-size:22px;font-weight:700;font-family:var(--mono);letter-spacing:-.03em;flex-shrink:0}
`;

// localStorage прокси — если браузер блокирует в blob-iframe
const lsProxy = `
(function(){
  // Активируем прокси если мы внутри iframe (shell) — тогда localStorage
  // изолирован (Blob/null origin) и данных там нет. Запрашиваем у родителя.
  var _inShell = (window !== window.top);
  var _ok = !_inShell;
  if(!_ok){
    try { localStorage.getItem('__test__'); } catch(e){ /* уже false */ }
  }
  if(_ok) return;
  // In-memory store + синк с shell через postMessage
  var _store = {};
  var _fake = {
    ...
    removeItem: function(k){ delete _store[k]; try{window.parent.postMessage({type:'DB_WRITE',key:k,value:null},'*');}catch(e){} },
  ...
  var _keys = ['ba_v2','bondan_refs','bondan_girbo_proxy','ba_apikey',
               'bondan_industry_peers','bondan_industry_medians','bondan_rosstat_ratios',
               'bondan_girbo_proxy','ba_sync_code','ba_gist_id','ba_gist_token'];
  _keys.forEach(function(k){
    window.parent.postMessage({type:'DB_READ',reqId:'ls_init_'+k,key:k},'*');
  });
  window.addEventListener('message',function(e){
    ...
    // Когда shell рассылает об изменении — обновляем и у себя
    if(d.type==='DB_NOTIFY' && d.key){
      window.parent.postMessage({type:'DB_READ',reqId:'ls_ntf_'+d.key,key:d.key},'*');
    }
  });
})();
`;
...
(function(){
  ...
  st.textContent = [
    ...
    '.score-comps{font-size:9px;color:var(--t3);font-family:var(--mono)}',
    // Фильтр-чипы из макета
    '.yr{padding:2px 7px;border:1px solid var(--brd);background:transparent;color:var(--t3);font-size:10px;font-family:var(--mono);font-weight:600;cursor:pointer;border-radius:0;transition:all .1s}',
    '.yr:hover{border-color:var(--acc);color:var(--acc)}',
    '.yr.active{background:rgba(255,0,128,0.08);border-color:rgba(255,0,128,0.4);color:var(--acc)}',
    '.ic{padding:2px 7px;border:1px solid var(--brd);background:transparent;color:var(--t3);font-size:9.5px;cursor:pointer;border-radius:0;transition:all .1s;line-height:1.4}',
    '.ic:hover{border-color:rgba(170,90,255,0.3);color:var(--t2)}',
    '.ic.active{background:rgba(170,90,255,0.1);border-color:rgba(170,90,255,0.45);color:var(--pur)}',
    '.rc{padding:2px 7px;border:1px solid var(--brd);background:transparent;color:var(--t3);font-size:10px;font-family:var(--mono);font-weight:600;cursor:pointer;border-radius:0;transition:all .1s}',
    '.rc:hover{border-color:rgba(170,90,255,0.35);color:var(--t2)}',
    '.rc.active{background:rgba(170,90,255,0.12);border-color:rgba(170,90,255,0.5);color:var(--pur)}',
    '.yr-chips,.rating-chips,.industry-chips{display:flex;flex-wrap:wrap;gap:3px}',
    // Секции фильтра (аккордеон в сайдбаре)
    '#rep-filter-details summary{font-size:9px!important;letter-spacing:.12em}',
    '#rep-filter-details select{font-size:10px!important;padding:4px 7px!important}',
    '#rep-filter-details label span:first-child{font-size:9px!important;letter-spacing:.09em!important}',
  ].join('');
...
function _repBuildDynPanel(iss){
  ...
  var fmtBn=function(v){
    if(v==null||!isFinite(v))return '—';
    var av=Math.abs(v);
    if(av>=1000) return (v/1000).toFixed(1)+' трлн ₽';
    if(av>=1)    return (av>=100?v.toFixed(0):av>=10?v.toFixed(1):v.toFixed(2))+' млрд ₽';
    if(av>=0.001)return (v*1000).toFixed(av>=0.1?0:1)+' млн ₽';
    if(av>0)     return (v*1000000).toFixed(0)+' тыс ₽';
    return '0';
  };
  ...
  function drow(lbl, key, bold, isDebtLike){
    ...
    var isGood=delta==null?null:(isDebtLike?delta<0:delta>0);
    var barColor=isGood===null?'':(isGood?'rgba(82,242,201,0.55)':'rgba(255,77,122,0.65)');
    var barSide=delta!=null?(delta>0?'left:50%':'right:50%')+';background:'+barColor:'';
  ...
  function rrow(lbl,val,prevVal,scaleMin,scaleMax,normVal,normDir,unit,lowerBetter){
    ...
    return '<div class="rrow"><div class="rrow-top">'
      ...
      +'<div class="r-fill" style="'+(lowerBetter?'left:0':'right:0;left:auto')+';width:'+fillPct+'%;background:'+fillColor+'"></div>'
      +(prevPct!=null?'<div class="r-prev" style="'+(lowerBetter?'left:'+prevPct+'%':'right:'+prevPct+'%;left:auto')+'"></div>':'')
...
// Статический фолбэк русских названий отраслей (на случай если _industryData ещё не загружен)
var _IND_LABELS_RU = {
  oil_gas:'Нефть и газ', metals_mining:'Металлы и добыча', metals_products:'Металлоизделия',
  machinery:'Машиностроение', auto_transport:'Автомобили, транспортная техника',
  electronics:'Электроника и электрооборудование', chemistry:'Химия и удобрения',
  pharma:'Фармацевтика', plastics:'Пластмассы и резина', construction_materials:'Стройматериалы',
  wood_paper:'Дерево, бумага, печать', textile:'Текстиль и одежда', furniture:'Мебель и прочее',
  agro_food:'Агро и пищевая', energy_utilities:'Электроэнергетика и ЖКХ',
  construction:'Строительство и девелопмент', real_estate:'Недвижимость', retail:'Ритейл и опт',
  transport:'Транспорт и логистика', hospitality:'Гостиницы и общепит', media:'Медиа и связь',
  telecom:'Телеком', it:'IT и разработка ПО', banks:'Банки', insurance:'Страхование',
  leasing:'Лизинг', mfo:'МФО и потребкредитование', holding:'Холдинги / SPV',
  prof_services:'Проф. услуги, консалтинг', science:'Наука и R&D',
  state:'Гос. органы и муниципалитеты', other:'Прочее'
};
function _indLbl(key){
  if(!key) return '—';
  var d=window._industryData;
  if(d&&d.industries&&d.industries[key]&&d.industries[key].label) return d.industries[key].label;
  return _IND_LABELS_RU[key]||key;
}
...
function _repRenderMfGrid(){
  ...
  grid.innerHTML=_MF_DEFS.map(function(d){
    ...
    return '<div class="'+cls+'" id="'+d.id+'" onclick="cycleMfSort(\\\''+d.id+'\\\')">'
      ...
      +'<input class="mf-in" placeholder="max" value="'+(rng.max!=null?rng.max:'')+'" onclick="event.stopPropagation()" oninput="_repMfOnInput(\\\''+d.id+'\\\')">'
...
function _repRenderIndChips(){
  ...
  var entries=Object.keys(inds).map(function(k){return [k,_indLbl(k)];});
