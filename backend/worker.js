...
async function handleCatalog(env){
  ...
  try {
    const r = await env.DB.prepare(`
      SELECT i.inn, i.short_name AS name, i.ticker, i.sector, i.kind, i.status,
             i.ogrn, i.okved, i.okved_name, i.region, i.bonds_count, i.aliases,
             CASE WHEN rmax.max_year IS NOT NULL THEN 1 ELSE 0 END AS has_reports,
             rmax.max_year AS latest_report_year,
             pa.parent_inn, pa.parent_name
      FROM issuers i
      LEFT JOIN (
        SELECT inn, MAX(fy_year) AS max_year FROM issuer_reports GROUP BY inn
      ) rmax ON rmax.inn = i.inn
      LEFT JOIN (
        SELECT child_inn, parent_inn,
               (SELECT short_name FROM issuers WHERE inn = a2.parent_inn) AS parent_name
        FROM issuer_affiliations a2
        WHERE role = 'founder' AND parent_kind = 'LEGAL' AND parent_inn != ''
        GROUP BY child_inn
...
async function girboFetchByInn(inn, maxYears = 5, opts){
  ...
  // ФНС ищет полнотекстово, но на разные слова реагирует по-разному:
  //   • полное имя «ФЕДЕРАЛЬНАЯ ГИДРОГЕНЕРИРУЮЩАЯ КОМПАНИЯ - РУСГИДРО»
  //     → 26 544 результата (по каждому слову бьёт), наш ИНН не в
  //     первой странице.
  //   • «гидрогенерирующая» → 0 (длинные специфические не индексируются).
  //   • «РусГидро» → 29 результатов, среди них наш ИНН.
  //
  // Стратегия: брендовое слово с конца имени — обычно компания имеет
  // в названии после ОПФ и описания собственный «бренд». Stop-list
  // для общих юр-слов (компания, общество, страхование и т.п.).
  const STOP_WORDS = new Set([
    'компания','компании','компаний','общество','организация','предприятие',
    'банк','банка','банковский','банковская',
    'российская','российский','российское','россии','россия',
    'федеральная','федеральный','федеральное',
    'государственная','государственный','государственное',
    'национальная','национальный','национальное',
    'инвестиционная','инвестиционный','инвестиции','страховая','страхование','страхования',
    'специализированное','специализированная','финансовое','финансовая','финансовый',
    'финанс','финансы','финансов',
    'фирма','завод','группа','группы','концерн','холдинг','корпорация','корпорации',
    'торгово','торговая','торговый','промышленная','промышленный',
    'производственная','производственное','производство',
    'акционерное','акционерная','акционерные','непубличное','непубличная',
    'публичное','публичная','общества','обществ','имени','имeни',
  ]);
  const lower = (s) => String(s || '').toLowerCase();
  const tokens = companyName.split(/[\s\-,«»"'\.()/]+/).filter(t => t.length >= 4);
  const brandTokens = tokens.filter(t => !STOP_WORDS.has(lower(t)));
  // Order: с конца → к началу (бренд чаще в хвосте имени).
  brandTokens.reverse();
  ...
  // Полное имя — последний резерв (обычно даёт слишком много)
  if(!queries.includes(companyName)) queries.push(companyName);
  // shortenIssuerName тоже в резерв
