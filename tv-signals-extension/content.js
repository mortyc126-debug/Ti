...
(function () {
  ...
  const DESC = {
    ...
    channel_level_fut: { what: 'Fade от кластер-уровня (медиана пивотов ±5 в 0.5 ATR, ≥3 касания) во флэте (ER<0.35), с гейтом методами расширения: сделка проходит только если donchian и twiggs не против направления, а liq_sweep и fractional_diff не за (сырые знаки). Брекет 1.5/0.75 ATR. Кап: skip если 2+ вложенных канала (30/80/200) пробиты на стороне fade.', read: 'Возврат от уровня ПО направлению отскока (лонг от поддержки, шорт от сопротивления) — но только когда 4-метод гейт подтверждает. Для фьючерсов на 1ч ТФ.', note: 'Порт стратегии channels_lab (invest-bot), единственная OOS-подтверждённая находка сессии: на 22 ликвидных фьючерсах train +0.092 / TEST +0.221 ATR/сделку по времени-split. На акциях edge ~0 → метод бесполезен вне фьючерсов, порог «элитно»/«строго» сам его там отсечёт (per-ticker exp ≈0).' },
  };
  ...
  const MARKET_STATS = {
    ...
    channel_level_fut: { d: null, win: null, n: null,  role: 'signal', note: 'ext-only · фьючерсы 1ч · train +0.092 / TEST +0.221 ATR (channels_lab OOS, 22 фьюча). На акциях ~0 — не для них' },
  };
