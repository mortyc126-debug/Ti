...
(function () {
  ...
  const META = [
    ...
    ['bb_keltner_squeeze', 'TTM Squeeze (инв.)', '#2ECC71'],
    ['adaptive_ma', 'KAMA-отклонение (инв.)', '#E74C3C'],
    ['fractional_diff', 'Дробный дифф (инв.)', '#9B59B6'],
    ['cumul_delta', 'Order Flow Δ (инв.)', '#1ABC9C'],
    ['rmi', 'RMI (off)', '#7F8C8D'],
    ['klinger', 'Klinger (off)', '#7F8C8D'],
    ['twiggs', 'Twiggs Money Flow (off)', '#7F8C8D'],
    ...
    ['channel_level_fut', 'Level-fade фьючерсы 1ч', '#00B4D8'],
  ];
  ...
  // Группы перегруппированы по актуальному combo-тесту invest-bot (180 дней, 400+
  // тикеров, 19 методов): БОТА combo_methods.py применён к тому же подмножеству,
  // что и в расширении. Правило: метод в «ядре синергий», если участвует хотя бы
  ...
  const GROUPS = [
    ...
    { title: 'MODS-обработка — инвертированы/режимные (по score_methods.py)', ids: ['accel', 'liq_sweep', 'alligator_inv', 'nw', 'bb_keltner_squeeze', 'adaptive_ma', 'fractional_diff', 'cumul_delta'],
      desc: 'По BASELINE (420 тикеров, июль 2026) + свежий top-50 ликвидных + walk_forward 12 окон: методы стабильно anti — их голос инвертируется в computeAll декларативно, чтобы стать signal. accel (PRICE_ACCEL walk-fw stable -0.05..-0.11), liq_sweep (LIQUIDITY_SWEEP d≈-0.10), alligator_inv (ALLIGATOR walk-fw stable -0.07..-0.18, инвертирован в имени). NW — режимный (⚙REG-INV): в drift, знак положительный в 10 из 12 окон, инверсия только в trending_down. Четвёрка bb_keltner_squeeze/adaptive_ma/fractional_diff/cumul_delta портирована по toggle_effect.py (invest-bot) — офлайн-замер эффекта method_toggle_state.json дал Δ WR +3.77 п.п. на всей вселенной, и эти четыре метода — крупнейшие индивидуальные вкладчики среди invert-кандидатов (BB_KELTNER_SQUEEZE walk-fw stable -0.07..-0.19, ADAPTIVE_MA -0.07..-0.30, FRACTIONAL_DIFF anti во всех прогонах, CUMUL_DELTA walk-fw stable -0.02..-0.10). anchored_vwap также anti, но инверсия внутри метода — он в первой группе.' },
    { title: '🎯 OOS-находки — спец. режимы (extension-only)', ids: ['ema200_revert', 'channel_level_fut'],
      desc: 'Событийные методы, портированные из invest-bot после честной train/test проверки. ema200_revert — mean-reversion к EMA200 при созревшем отрыве (ER-гейт + про-инструментальный фильтр по частоте касаний). channel_level_fut — fade от кластер-уровня во флэте с гейтом 5 методами (donchian+twiggs+klinger согласны, liq_sweep+fractional_diff не против), брекет 2.0/1.0; единственная OOS-подтверждённая находка сессии по каналам: на 22 ликвидных фьючерсах 1ч train +0.312 / TEST +0.518 ATR/сделку (тонкая выборка n≈82; на акциях ~0 — порог «элитно» сам отсечёт). Оба редкие, но качественные — включать на 1ч ТФ, ловить порогами strict/elite.' },
    { title: '⛔ Отключены (noise/шум по BASELINE + walk_forward)', ids: ['elliott_wave', 'rmi', 'klinger', 'twiggs', 'donchian', 'wick_rejection', 'level_quality'],
      desc: 'ELLIOTT_WAVE — walk_forward показал знак d, пляшущий между окнами (+0.15/+0.13/-0.24/-0.16/+0.02/-0.66) — случайный шум, инверсия не спасает. RMI/KLINGER/TWIGGS/DONCHIAN/WICK_REJECTION/LEVEL_QUALITY — 6 disable-кандидатов из method_toggle_state.json бота: в BASELINE везде нейтраль, толку от их голоса нет. По toggle_effect.py их отключение освобождает вес для сигнальных методов (наибольший вклад: ALLIGATOR -20984, RMI -16538, KLINGER -16074 к весовому Δ WR, если бы остались голосовать). Голос заменён нулями и не участвует в composite. Ярлык «off».' },
  ];
  ...
  const DESC = {
    ...
    dfa_regime: { what: 'Detrended Fluctuation Analysis (Peng 1994): α-коэффициент масштабной инвариантности ряда доходностей. α>0.65 = persistent (long memory), α<0.35 = anti-persistent (mean-reversion), 0.35-0.65 = близко к белому шуму.', read: 'α>0.65 + недавнее движение → сигнал ПО тренду (продолжение). α<0.35 + движение → ПРОТИВ (mean-reversion). Между 0.35 и 0.65 — молчит.', note: 'САМЫЙ СИЛЬНЫЙ из новых по 180-дневному бэктесту бота: d=+0.178, win 55.5%, 47k срабатываний/325 тикеров. Топ-синергия: с BIPOWER_JUMP (+1.57 lift, 90% согласия), CASCADE (+0.60), TALIB_ANTI (+0.54). Ортогонально Hurst (у нас в FRACTAL): DFA устойчив к нестационарным рядам.' },
    bipower_jump: { what: 'Barndorff-Nielsen-Shephard разложение realized variance на непрерывный и скачковой компоненты. jump_ratio = max(0, RV-BV)/RV; BV = (π/2)·Σ|r_i·r_{i-1}| устойчив к скачкам.', read: 'jump_ratio > 0.4 + сильное недавнее направление → сигнал ПРОТИВ (скачки часто overshoot, за ними mean-reversion).', note: 'd=+0.164 win 54.5% / 17k срабатываний. Топ-синергия: с ELLIOTT_WAVE (+1.62 lift, 21% ~ редкий-но-сильный), DFA_REGIME (+1.57, 90% ~ частый-и-сильный). В споре с PRICE_ACCEL: верь BIPOWER_JUMP (+0.83 t=+8.4).' },
    amihud_shock: { what: 'Amihud illiquidity = |return|/volume — прокси Kyle\'s lambda (price impact на единицу объёма). Институциональная классика для sizing.', read: 'Текущий Amihud > 2× медианы длинного окна + недавнее движение → сигнал ПРОТИВ движения ("цена сдвинулась слишком мало на объём" = ликвидность сдохла, разворот вероятен).', note: 'd=+0.144 win 54.7%, ОЧЕНЬ ЧАСТЫЙ (98k срабатываний/385 тикеров). Крупнейший вклад среди новых. Синергии: с VPIN_TOXICITY (+1.27), CASCADE (+0.71), TALIB_ANTI (+0.43). Дубль с ZSCORE (99% agr, lift 0) — один из двух держать активным.' },
    vpin_toxicity: { what: 'VPIN (Easley-López de Prado) через Bulk Volume Classification: свечи агрегируются в объёмные бакеты, buy_frac оценивается через CDF нормали от нормированной доходности. VPIN = |buy-sell|/V, среднее по последним 15 бакетам.', read: 'Перцентиль VPIN > 75% истории + движение > 0.5σ → сигнал ПРОТИВ движения (informed flow "съел" движение, продолжения не будет).', note: 'd=+0.127 win 49.7%, 9.6k срабатываний. Топ-синергия: с ZSCORE (+1.22 lift), FVG (+0.51), HAWKES (+0.47). Институциональный tool для детекции токсичного потока перед резкими движениями (flash crash indicator).' },
    ...
    elliott_wave: { what: 'Эвристический счёт импульсной 5-волновки через ATR-зигзаг пивотов. Жёсткие правила: волна 2 не пробивает P0, волна 3 не короче волны 1, волна 4 не заходит на территорию волны 1, ретрейс волны 2 в 23.6-88.6% Фибо-зоне.', read: 'Знак пляшет между окнами walk_forward — метод ОТКЛЮЧЁН, голос не участвует в composite.', note: 'walk_forward (12×90д): +0.15/+0.13/-0.24/-0.16/+0.02/-0.66 — единственный noise-метод из 54 проверенных. Инверсия не спасала (пробовали раньше) — знак случаен по периодам, не только по амплитуде. Ярлык «off».' },
    bb_keltner_squeeze: { what: 'TTM Squeeze: Bollinger Bands (20, 2σ) внутри Keltner Channel (EMA20, 1.5×ATR14) = компрессия волатильности. Выход из сжатия + TTM-momentum (close vs mid донченовского канала за 20 баров) даёт направление.', read: 'ИНВЕРТИРОВАН (walk-fw stable anti -0.07..-0.19): выход из сжатия по моментуму → сигнал ПРОТИВ направления momentum (истинный выброс чаще против первичного импульса на 5-мин РФ).', note: 'Портирован по toggle_effect.py: второй по вкладу invert-кандидат (+9802 к весовому Δ WR в offline-замере). Упрощён без duration_mult (второстепенный множитель силы по 40-барной истории BB-std — знак не меняет).' },
    adaptive_ma: { what: 'Отклонение цены от KAMA (Kaufman Adaptive MA, Efficiency Ratio period=10), нормировано на 200-барную волатильность, tanh-сжатие.', read: 'ИНВЕРТИРОВАН (walk-fw stable anti -0.07..-0.30, самый сильный anti из новых): цена выше KAMA → сигнал ВНИЗ, ниже → ВВЕРХ (mean-reversion к адаптивной средней, не тренд-следование).', note: 'Крупнейший индивидуальный вклад среди invert-4 по toggle_effect.py (+6973 к весовому Δ WR). Базовая формула без ER-мультипликатора и дивергенции полной _candle-версии бота — те модулируют амплитуду, не знак.' },
    fractional_diff: { what: 'Дробное дифференцирование ряда цен (d=0.4): веса w_k=(-1)^k·C(d,k), обрыв при |w|<1e-4 (окно до 40 баров). Стационарнее чем цены, но сохраняет долгосрочную память в отличие от обычной разности (d=1).', read: 'ИНВЕРТИРОВАН (BASELINE anti -0.063 глобально): знак frac-diff серии + её наклон, усиленный при совпадении с ускорением → сигнал ПРОТИВ.', note: 'Третий по вкладу invert-кандидат (+5084 к весовому Δ WR). В top-50 попадал в drift (сила гуляла между окнами), но знак ни разу не менялся — устойчивая anti.' },
    cumul_delta: { what: 'Накопленный order-flow прокси: Σ объём×знак(close-open)×min(1, 2×тело/размах) за окно ~1.5ч (18×5м), нормировано в диапазон окна [-1..1].', read: 'ИНВЕРТИРОВАН (walk-fw stable anti -0.02..-0.10): накопленная дельта покупателей/продавцов → сигнал ПРОТИВ доминирующей стороны (агрессия исчерпывается, а не продолжается).', note: 'В первичном BASELINE был noise (d=-0.045) — walk_forward и top-50 показали стабильную (пусть и слабую) anti во всех 12 окнах. +3940 к весовому Δ WR по toggle_effect.py.' },
    rmi: { what: 'Relative Momentum Index — вариант RSI на разностях close[i]-close[i-momentum] вместо close[i]-close[i-1] (сглаженнее обычного RSI).', read: 'Метод ОТКЛЮЧЁН — голос не участвует в composite.', note: 'В BASELINE везде нейтраль/шум — один из 8 методов в disabled бота (data/method_toggle_state.json). По toggle_effect.py — крупнейший вклад среди disable-кандидатов (-16538 к весовому Δ WR, если бы остался голосовать).' },
    klinger: { what: 'Klinger Volume Oscillator: денежный поток через направление hlc-тренда и истинный диапазон свечи (не просто знак объёма, как OBV). EMA(34)-EMA(55) от volume force.', read: 'Метод ОТКЛЮЧЁН — голос не участвует в composite.', note: 'В BASELINE везде нейтраль. -16074 к весовому Δ WR по toggle_effect.py если бы остался голосовать — освобождённый вес идёт сигнальным методам.' },
    twiggs: { what: 'Twiggs Money Flow: True Range Value весит объём направлением закрытия внутри истинного диапазона (max(high,prev_close)..min(low,prev_close)), EMA(21) нормировка.', read: 'Метод ОТКЛЮЧЁН — голос не участвует в composite.', note: 'В BASELINE везде нейтраль. -14507 к весовому Δ WR по toggle_effect.py если бы остался голосовать.' },
    donchian: { what: 'Асимметрия касаний краёв 20-барного канала Дончиана — только в боковике (диапазон < 4%). Плотный край = скопление стопов/позиций → выброс идёт в менее плотную сторону.', read: 'Метод ОТКЛЮЧЁН — голос не участвует в composite.', note: 'В BASELINE везде нейтраль (d≈-0.001 глобально). -13627 к весовому Δ WR по toggle_effect.py.' },
    wick_rejection: { what: 'Дисбаланс верхних/нижних хвостов свечей за окно ~2ч, взвешенный на объём и на "маленькое тело → хвост важнее".', read: 'Метод ОТКЛЮЧЁН — голос не участвует в composite.', note: 'В BASELINE везде нейтраль. -12264 к весовому Δ WR по toggle_effect.py.' },
    level_quality: { what: 'Confluence-скор уровня: сколько независимых признаков (order block, POC/HVN) видят цену рядом. В боте — 5 критериев (+weekly open, significant H/L, second touch); здесь упрощено до 2 (не имеют внутридневного смысла и метод всё равно disabled).', read: 'Метод ОТКЛЮЧЁН — голос не участвует в composite.', note: 'В BASELINE везде нейтраль. -9559 к весовому Δ WR по toggle_effect.py.' },
    channel_level_fut: { what: 'Fade от кластер-уровня (медиана пивотов ±5 в 0.5 ATR, ≥3 касания) во флэте (ER<0.35), с гейтом 5 методами расширения: сделка проходит только если donchian, twiggs и klinger не против направления, а liq_sweep и fractional_diff не за (сырые знаки). Брекет 2.0/1.0 ATR (R:R 2:1). Кап: skip если 2+ вложенных канала (30/80/200) пробиты на стороне fade.', read: 'Возврат от уровня ПО направлению отскока (лонг от поддержки, шорт от сопротивления) — но только когда 5-метод гейт подтверждает. Для фьючерсов на 1ч ТФ.', note: 'Порт стратегии channels_lab (invest-bot), единственная OOS-подтверждённая находка сессии: на 22 ликвидных фьючерсах train +0.312 / TEST +0.518 ATR/сделку (split, гейт с klinger, брекет 2.0/1.0). Выборка тонкая (test n≈82) — метод редкий. На акциях edge ~0 → порог «элитно»/«строго» сам отсечёт (per-ticker exp ≈0).' },
  };
  ...
  // desktop-уведомление о новом сигнале — через мост в isolated-world (MAIN не
  // видит chrome.notifications). Fire-and-forget, ответ не ждём.
  ...
  // на время скана видно, как график листает тикеры — это ожидаемо, не глюк.
  function _chartSetSymbol(code) {
    return new Promise(resolve => {
      let done = false; const finish = () => { if (!done) { done = true; resolve(); } };
      try { S.chart.setSymbol(code, finish); } catch (e) { finish(); } // callback есть не во всех сборках — подстрахован таймаутом
      setTimeout(finish, 4000);
    });
  }
  async function scanFetchViaChart(code) {
    await _chartSetSymbol(code);
    let waited = 0; // страховка: exportData() может ещё отдавать ПРЕЖНИЙ тикер, пока symbol() не сменился
    while (waited < 3000) {
      let sym = ''; try { sym = S.chart.symbol(); } catch (e) {}
      if ((sym || '').split(':').pop().toUpperCase() === code) break;
      await new Promise(r => setTimeout(r, 200)); waited += 200;
    }
    await new Promise(r => setTimeout(r, 400)); // дать свечам догрузиться после смены символа
    try {
      const res = await Promise.resolve(S.chart.exportData());
      const bars = window.SignalsCore.parseExport(res);
      if (bars.length >= 80) {
        const gaps = []; for (let i = 1; i < bars.length; i++) gaps.push(bars[i].time - bars[i - 1].time);
        gaps.sort((a, b) => a - b);
        return { ok: true, bars, dtSec: gaps[gaps.length >> 1] || S.barDt || 300 };
      }
    } catch (e) {}
    return { ok: false };
  }
  async function scanRun(isAuto) {
    ...
    const source = ((document.getElementById('tvsig-scan-src') || {}).value) || 'chart';
    ...
    const processTicker = (code, bars, dtSec) => {
      ...
      if (auto) {
        ...
        let comp; try { comp = SC.computeAll(bars, 12); } catch (e) { return; }
      ...
      let sa, sb;
      try { sa = SC.methods[A](bars); sb = B && B !== '-' ? SC.methods[B](bars) : null; } catch (e) { return; }
    ...
    if (source === 'chart') {
      if (!S.chart) { body.innerHTML = '<div class="tvsig-fc-hint">График терминала не найден — открой «Модели» и дождись подключения, или выбери источник MOEX.</div>'; return; }
      const origSymbol = S.symbol || (function () { try { return S.chart.symbol(); } catch (e) { return ''; } })();
      try {
        for (let idx = 0; idx < codes.length; idx++) {
          const code = codes[idx];
          body.innerHTML = '<div class="tvsig-fc-hint">Сканирую через график терминала: ' + (idx + 1) + '/' + codes.length + ' — ' + code + '…</div>';
          let f; try { f = await scanFetchViaChart(code); } catch (e) { f = null; }
    ...
    if (isAuto) notifyCheckScan(hits);
  ...
  // desktop-уведомления о новых хитах сканера — только на авто-тиках (ручной
  // клик пользователь и так видит своими глазами). Как и с методами тикера,
  // первый авто-тик после включения ничего не шлёт (S._scanNotifyPrev ещё
  ...
  function scanRender(hits, A, B, look, diag, auto, meta) {
    ...
    // MOEX ограничен 5 фиксированными интервалами (1/10/60/1440/10080 мин) — соседние
    // ТФ графика могут схлопнуться в один и тот же, отсюда одинаковые хиты на разных ТФ.
  ...
  async function indexEnsure(bars, dt) {
    ...
    if (S._indexKey === key && S._indexAligned) return S._indexAligned;
    if (S._indexBuilding === key) return null;
    S._indexBuilding = key;
    const iv = dt <= 90 ? 1 : dt <= 1200 ? 10 : dt <= 10800 ? 60 : dt <= 259200 ? 24 : 7;
    const ivSec = iv === 1 ? 60 : iv === 10 ? 600 : iv === 60 ? 3600 : iv === 24 ? 86400 : 604800;
  ...
  function _pairReturns(aBars, idxCloses, from) {
    const rA = [], rI = [];
    for (let i = Math.max(1, from); i < aBars.length; i++) {
      const a0 = aBars[i - 1].close, a1 = aBars[i].close;
      const i0 = idxCloses[i - 1], i1 = idxCloses[i];
      if (a0 == null || a1 == null || i0 == null || i1 == null || a0 <= 0 || i0 <= 0) continue;
      rA.push(a1 / a0 - 1); rI.push(i1 / i0 - 1);
    }
    return { rA, rI };
  }
  // Полный индексный контекст: короткая/длинная corr, beta, лид-лаг, %-ход
  // индекса и актива за N бар, idiosync (актив минус beta·индекс).
  function computeIndexContext(bars, idxCloses) {
    if (!bars || !idxCloses || bars.length < 30) return null;
    const n = bars.length;
    const shortN = 20, longN = 100, moveN = 12;
    const short = _pairReturns(bars, idxCloses, Math.max(1, n - shortN));
    const long  = _pairReturns(bars, idxCloses, Math.max(1, n - longN));
    if (short.rA.length < 8 || long.rA.length < 20) return null;
    const corrShort = _pearson(short.rA, short.rI);
    const corrLong  = _pearson(long.rA,  long.rI);
    // beta: cov(A,I)/var(I) на длинном окне
    const mI = long.rI.reduce((a, b) => a + b, 0) / long.rI.length;
    const mA = long.rA.reduce((a, b) => a + b, 0) / long.rA.length;
    let cov = 0, varI = 0;
    for (let i = 0; i < long.rI.length; i++) {
      const dI = long.rI[i] - mI, dA = long.rA[i] - mA;
      cov += dA * dI; varI += dI * dI;
    }
    const beta = varI > 0 ? cov / varI : null;
    // лид-лаг: где корреляция максимальна при сдвиге актива относительно индекса
    // (актив на k баров РАНЬШЕ → актив.i vs индекс.i-k). k>0 → актив опережает.
    let bestLag = 0, bestAbs = corrShort == null ? 0 : Math.abs(corrShort);
    for (let k = -3; k <= 3; k++) {
      if (k === 0) continue;
      const rA2 = [], rI2 = [];
      for (let i = Math.max(1, n - shortN); i < n; i++) {
        const ai = i - k; if (ai <= 0 || ai >= n) continue;
        const a0 = bars[ai - 1].close, a1 = bars[ai].close;
        const i0 = idxCloses[i - 1], i1 = idxCloses[i];
        if (a0 == null || a1 == null || i0 == null || i1 == null || a0 <= 0 || i0 <= 0) continue;
        rA2.push(a1 / a0 - 1); rI2.push(i1 / i0 - 1);
      }
      if (rA2.length < 8) continue;
      const c = _pearson(rA2, rI2);
      if (c != null && Math.abs(c) > bestAbs) { bestAbs = Math.abs(c); bestLag = k; }
    }
    // ход индекса и актива за moveN баров (%)
    let idxFrom = null, idxTo = null, actFrom = null, actTo = null;
    for (let i = n - 1; i >= Math.max(0, n - moveN); i--) {
      if (idxCloses[i] != null && idxTo == null) idxTo = idxCloses[i];
      if (idxCloses[i] != null) idxFrom = idxCloses[i];
      if (bars[i] && actTo == null) actTo = bars[i].close;
      if (bars[i]) actFrom = bars[i].close;
    }
    const idxMove = (idxFrom && idxTo) ? (idxTo / idxFrom - 1) : null;
    const actMove = (actFrom && actTo) ? (actTo / actFrom - 1) : null;
    // idiosync за moveN: актив − beta · индекс (что «сам» сделал актив)
    const idiosync = (actMove != null && idxMove != null && beta != null) ? (actMove - beta * idxMove) : null;
    return { corrShort, corrLong, beta, bestLag, bestLagAbs: bestAbs, idxMove, actMove, idiosync };
  }
  // Вердикт согласия/противоречия сигнала с индексом (для бейджа рядом с методом).
  // Возвращает 'agree' | 'against' | 'neutral' | 'weak' (низкая corr — индекс не
  // информативен для этого тикера).
  function _signalVsIndex(dir, ctx) {
    if (!dir || !ctx) return null;
    const c = ctx.corrShort, im = ctx.idxMove;
    if (c == null || im == null) return null;
    // низкая корреляция — индекс не диктует
    if (Math.abs(c) < 0.2) return 'weak';
    const idxDir = Math.sign(im);
    if (idxDir === 0) return 'neutral';
    // при положительной corr согласие = одинаковые знаки; при отрицательной — противоположные
    const expected = c > 0 ? idxDir : -idxDir;
    return dir === expected ? 'agree' : 'against';
  }
  ...
  function scanAutoSync() {
    ...
    S._scanAutoTimer = setInterval(() => { if (!S._scanBusy) scanRun(true); }, secs * 1000);
  }
  function scanInit() {
  ...
  function _indexBadge() {
    ...
    const idxPct = ctx.idxMove != null ? ((ctx.idxMove >= 0 ? '+' : '') + (ctx.idxMove * 100).toFixed(2) + '%') : '—';
    const corr = ctx.corrShort;
    let corrTxt = '—', corrCls = 'dim';
    if (corr != null) {
      corrTxt = corr.toFixed(2);
      corrCls = Math.abs(corr) < 0.2 ? 'dim' : Math.abs(corr) > 0.5 ? 'strong' : '';
    }
  ...
  function forecastRender() {
    ...
    if (rgEl) rgEl.innerHTML = _regimeBadge(rg) + _indexBadge();
  ...
  const MARKET_STATS = {
    ...
    // топ-10 из toggle_effect.py (offline-замер эффекта method_toggle_state.json,
    // Δ WR +3.77 п.п. на ALL) — n здесь взвешенный n_fires из CSV, не сырой счётчик
    bb_keltner_squeeze: { d: -0.175, win: 0.416, n: 12831, role: 'anti',  note: 'walk-fw stable, инвертирован, +9802 вклад по toggle_effect' },
    adaptive_ma:        { d: -0.280, win: 0.395, n: 4131,  role: 'anti',  note: 'сильнейший anti из новых, +6973 вклад по toggle_effect' },
    fractional_diff:    { d: -0.119, win: 0.456, n: 12995, role: 'anti',  note: 'anti во всех прогонах, +5084 вклад по toggle_effect' },
    cumul_delta:        { d: -0.081, win: 0.443, n: 14638, role: 'anti',  note: 'был noise в BASELINE, walk-fw подтвердил anti, +3940 вклад' },
    rmi:                { d:  0.073, win: 0.509, n: 14184, role: 'noise', note: 'нейтраль в BASELINE, ОТКЛЮЧЁН, -16538 вклад если бы голосовал' },
    klinger:            { d: -0.006, win: 0.472, n: 13944, role: 'noise', note: 'нейтраль в BASELINE, ОТКЛЮЧЁН, -16074 вклад если бы голосовал' },
    twiggs:             { d:  0.002, win: 0.471, n: 12373, role: 'noise', note: 'нейтраль в BASELINE, ОТКЛЮЧЁН, -14507 вклад если бы голосовал' },
    donchian:           { d: -0.012, win: 0.475, n: 12240, role: 'noise', note: 'нейтраль в BASELINE, ОТКЛЮЧЁН, -13627 вклад если бы голосовал' },
    wick_rejection:     { d:  0.048, win: 0.495, n: 10626, role: 'noise', note: 'нейтраль в BASELINE, ОТКЛЮЧЁН, -12264 вклад если бы голосовал' },
    level_quality:      { d:  0.023, win: 0.491, n: 11132, role: 'noise', note: 'нейтраль в BASELINE, ОТКЛЮЧЁН, -9559 вклад если бы голосовал' },
    ...
    channel_level_fut: { d: null, win: null, n: null,  role: 'signal', note: 'ext-only · фьючерсы 1ч · гейт 5 методов (+klinger) · брекет 2.0/1.0 · train +0.312 / TEST +0.518 ATR (channels_lab OOS split, n≈82). На акциях ~0 — не для них' },
  };
  ...
  // ── desktop-уведомления о новых сигналах текущего тикера ────────────────────
  // Сравнивает знак c[id].last с прошлым расчётом (S._notifyPrev, обнуляется
  // при смене тикера в refresh()) — уведомляет только на ПЕРЕХОД в новый знак,
  // не на каждый тик, пока сигнал держится. Первый расчёт после смены тикера
  // ничего не шлёт (S._notifyPrev ещё null) — иначе шквал уведомлений про уже
