...
(function () {
  ...
  const GROUPS = [
    ...
    { title: '🎯 OOS-находки — спец. режимы (extension-only)', ids: ['ema200_revert', 'channel_level_fut'],
      desc: 'Событийные методы, портированные из invest-bot после честной train/test проверки. ema200_revert — mean-reversion к EMA200 при созревшем отрыве (ER-гейт + про-инструментальный фильтр по частоте касаний). channel_level_fut — fade от кластер-уровня во флэте с гейтом 4 методами (donchian+twiggs согласны, liq_sweep+fractional_diff не против), брекет 2.0/1.0; единственная OOS-подтверждённая находка сессии по каналам: на 22 ликвидных фьючерсах 1ч train +0.374 / TEST +0.318 ATR/сделку (на акциях ~0 — порог «элитно» сам отсечёт). Оба редкие, но качественные — включать на 1ч ТФ, ловить порогами strict/elite.' },
  ...
  const DESC = {
    ...
    channel_level_fut: { what: 'Fade от кластер-уровня (медиана пивотов ±5 в 0.5 ATR, ≥3 касания) во флэте (ER<0.35), с гейтом методами расширения: сделка проходит только если donchian и twiggs не против направления, а liq_sweep и fractional_diff не за (сырые знаки). Брекет 2.0/1.0 ATR (R:R 2:1). Кап: skip если 2+ вложенных канала (30/80/200) пробиты на стороне fade.', read: 'Возврат от уровня ПО направлению отскока (лонг от поддержки, шорт от сопротивления) — но только когда 4-метод гейт подтверждает. Для фьючерсов на 1ч ТФ.', note: 'Порт стратегии channels_lab (invest-bot), единственная OOS-подтверждённая находка сессии: на 22 ликвидных фьючерсах train +0.374 / TEST +0.318 ATR/сделку (брекет 2.0/1.0 по TP-гриду — монотонно лучше узкого 1.5/0.75, где было +0.221). На акциях edge ~0 → порог «элитно»/«строго» сам отсечёт (per-ticker exp ≈0).' },
  };
  ...
  const MARKET_STATS = {
    ...
    channel_level_fut: { d: null, win: null, n: null,  role: 'signal', note: 'ext-only · фьючерсы 1ч · брекет 2.0/1.0 · train +0.374 / TEST +0.318 ATR (channels_lab OOS, 22 фьюча). На акциях ~0 — не для них' },
  };
