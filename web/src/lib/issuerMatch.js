...
// Короткое читаемое имя для списков/графиков: длинные обороты сокращаем,
// ведущую орг-форму и кавычки убираем — чтобы отличать длинные SPV-названия
// («ООО Специализированное финансовое общество …» → «СФО …»). Полное имя
// показываем в подсказке (title).
export function shortIssuerName(s){
  let x = String(s || '').trim();
  if(!x) return '';
  x = x
    .replace(/Специализированн\w*\s+финансов\w*\s+обществ\w*/gi, 'СФО')
    .replace(/Специализированн\w*\s+обществ\w*\s+проектн\w*\s+финансирован\w*/gi, 'СОПФ')
    .replace(/^(ПАО|ОАО|ЗАО|АО|ООО|НАО|НКО|АКБ|КБ)\s+/i, '')
    .replace(/[«»"“”„]+/g, '')
    .replace(/\s{2,}/g, ' ')
    .trim();
  return x || String(s);
